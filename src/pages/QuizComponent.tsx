...
const QuizComponent = () => {
  ...
  const [quizDescription, setQuizDescription] = useState<string | undefined>("");
  ...
  const getQuizData = async () => {
    try {
      const doc = await getQuizById(id);
      ...
      const quizData = quizDetails.map(item=> {
        const answer = [...item.answer].map(x=> ({
          selected: false,
          ...x, 
        }));
        ...
        <div className="bg-gray-50 h-20 w-full fixed bottom-0 sm:px-8 px-4 border border-t">
          <div className="flex justify-between items-center h-full">
            ...
            <div className="flex gap-2">
              ...
              >
                <button
                  className="rounded-md py-2 px-4 border border-transparent text-center text-sm text-white transition-all shadow-md hover:shadow-lg focus:bg-slate-700 focus:shadow-none active:bg-slate-700 hover:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none ml-2"
                  type="button"
              ...
              <div className={status === Status.Review ? "hidden" : "block"}>
                ...
                >
                  <button
                    className="rounded-md bg-slate-800 bg py-2 px-4 border border-transparent text-center text-sm text-white transition-all shadow-md hover:shadow-lg focus:bg-slate-700 focus:shadow-none active:bg-slate-700 hover:bg-slate-700 active:shadow-none disabled:pointer-events-none disabled:opacity-50 disabled:shadow-none ml-2"
                    type="button"
                    disabled={!quizAnswered[qIndex]}
                    onClick={() => {
                      if (status === Status.Complete) {
